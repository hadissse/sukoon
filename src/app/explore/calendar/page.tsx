...
// Scr220 — تقويم العبورات (شهري) — Apple Calendar-inspired month view.
...
function MonthGrid({ selected, onSelect }: { selected: number; onSelect: (d: number) => void }) {
  // May 2026 — May 1 falls on Friday → weekday index = 5 (Sun=0)
  ...
  return (
    <div className="bg-white rounded-2xl pt-3 px-2 pb-3.5" style={{ border: '1px solid #E8E2D2' }}>
      <div className="grid grid-cols-7 pb-2 mb-1" style={{ borderBottom: '1px solid #E8E2D2' }}>
        {WEEK_DAYS.map((d, i) => (
          <div key={i} className="text-center text-[10px] text-ink-muted font-semibold tracking-wide">
            {d}
          </div>
        ))}
      </div>
      <div className="grid grid-cols-7" style={{ rowGap: 2 }}>
        {cells.map((d, i) => {
          ...
          return (
            ...
            >
              <div
                className="w-7 h-7 rounded-[14px] flex items-center justify-center text-sm font-serif"
                style={{
  ...
  return (
    <div className="max-w-[430px] mx-auto w-full pb-28">
      ...
      <div className="px-4 mt-1">
        <div className="flex justify-between items-center mx-1 mb-2.5">
          <div className="flex items-baseline gap-2">
            <div className="font-serif text-[22px] text-ink font-medium">مايو ٢٠٢٦</div>
            <div className="text-[11px] text-ink-muted font-mono">ذو القعدة ١٤٤٧</div>
          </div>
          <div className="flex gap-1 items-center">
            <span className="px-2.5 py-1.5 text-base text-ink-muted">‹</span>
            <button onClick={() => setSelected(CAL_TODAY)} className="px-3 py-1.5 rounded-full bg-cream-soft text-xs text-coral font-semibold">
              اليوم
            </button>
            <span className="px-2.5 py-1.5 text-base text-ink-muted">›</span>
          </div>
        ...
        {/* legend */}
        <div className="flex gap-3 justify-center flex-wrap mt-3 px-1">
          {LEGEND.map(([label, c]) => (
        ...
        {/* selected day events */}
        <div className="mt-4 pt-3.5" style={{ borderTop: '1px solid #E8E2D2' }}>
          ...
          {events.length === 0 ? (
            <div
              className="p-[18px] bg-white rounded-xl text-center text-[13px] text-ink-muted"
              style={{ border: '1px dashed #E8E2D2' }}
            >
              يوم هادئ على خريطتك.
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              {selected === CAL_TODAY ? (
                <Link href="/explore/calendar/day">
                  <CalEventRow e={events[0]} />
                </Link>
              ) : (
                <CalEventRow e={events[0]} />
              )}
